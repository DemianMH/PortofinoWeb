// src/components/GiveawayForm.tsx
'use client'
...
export const GiveawayForm = () => {
  ...
  const [physicalTicket, setPhysicalTicket] = useState(''); // NUEVO: Para el ticket físico
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Actualizamos la validación para incluir el nuevo campo de ticket
    ...
    try {
      const response = await fetch('/api/enter-giveaway', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, phone, ticket: physicalTicket }), // Enviamos el ticket ingresado
      });
  ...
  return (
    <div className="bg-black/60 backdrop-blur-md rounded-xl shadow-lg p-8 w-full text-center">
      ...
      {!submitted ? (
        <>
          <h2 className="text-3xl font-bold mb-2 text-white">Win a Dinner for Two!</h2>
          <p className="text-gray-200 mb-6">Enter your details and ticket number for a chance to win.</p>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* NUEVO CAMPO PARA EL TICKET FÍSICO */}
            ...
            <button type="submit" disabled={loading} className="bg-yellow-500 text-gray-900 font-bold p-3 rounded-md hover:bg-yellow-400 transition-colors duration-300 disabled:bg-gray-500">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          {/* ELIMINAMOS la muestra del ticket generado */}
        </motion.div>
      )}
    </div>
  );
